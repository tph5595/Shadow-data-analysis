...
class PrivacyScope:
    ...
    def _map_row(self, row, ip_map, cache):
        ip = row['ip.src']
        five_tuple = (ip, row['ip.dst'], row['tcp.srcport'], row['tcp.dstport'], row['ip.proto'])
    ...
    def ip_map(self, src_map, dst_map):
        ...
        df['ip.src'] = df.apply(lambda row: self._map_row(row, src_map, five_tuple_cache), axis=1)
        df['ip.dst'] = df.apply(lambda row: self._map_row(row, dst_map, five_tuple_cache), axis=1)
        self.df = df
