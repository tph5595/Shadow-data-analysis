...
# resolv_df_filtered = resolv_df[resolv_df['ip.proto'] == DNS_PORT]
...
print("first pass: " + str(first_pass['dns.qry.name'].unique()))
...
def get_chat_logs(scope):
    ...
    for user in users:
        ...
        numeric_cols = client_log[user].select_dtypes(include=[np.number])
        no_var_cols = numeric_cols.columns[(numeric_cols.std() <= 0.01) | (numeric_cols.std().isna())].tolist()
    return client_log
...
# remove no variance features from src and dst
# for ip in src_df:
#     number_cols = src_df[ip].select_dtypes(include=[np.number])
#     cols_to_drop = number_cols.columns[(number_cols.std() <= 0.01) | (number_cols.std().isna())].tolist()
#     print("Dropping [src_df] " + str(len(cols_to_drop)) + " columns from " + str(ip))
#     src_df[ip] = src_df[ip].drop(columns=cols_to_drop, axis=1)

# for user in dst_df:
#     number_cols = dst_df[user].select_dtypes(include=[np.number])
#     cols_to_drop = number_cols.columns[(number_cols.std() <= 0.01) | (number_cols.std().isna())].tolist()
#     print("Dropping [dst_df] " + str(len(cols_to_drop)) + " columns from " + str(user))
#     dst_df[user] = dst_df[user].drop(columns=cols_to_drop, axis=1)
