...
def get_GNS3_offset():
    # get gns3 offset from service log
    with open('../data/server-log/pythonServerThread.stdout', 'r') as file:
        first_line = file.readline()
        time = first_line.split("\t")[0]
...
# set isp filter
ISP_scope.set_filter(isp_filter)
...
# ISP (set only ip search, cache search, not sure if anything is cached...)
ISP_scope.ip_search_enabled = True
...
scopes = [resolver, root, tld, sld, ISP_scope]
...
#             assert dst_arr[single_user].ndim == 2
            ...
            best_features = iterate_features(src_df, dst_df, n, features, tda_config,
                                            "./with-isp-without-shadow/" + "chatlog_tda_match_dns_all_" + str(n) +
