...
class PrivacyScope:
    ...
    def __init__(self, filenames, name):
        ...
        self.cache_timing = pd.Timedelta("30 seconds") # handle scaling of 10
    ...
    def adjust_time_without_scaling(self):
        df = self.as_df()
        df[self.time_col] = df[self.time_col].apply(lambda x: int(x.timestamp())) # convert to epoch
        col = df[self.time_col]
        self.df = df
        self.time_format = 'epoch'
        self.format_time_col()
...
#    tmp = df.index.to_frame()
    ...
    tmp = df.resample('100ms').sum(numeric_only=True).infer_objects() # changed to 100ms
    tmp = tmp.reset_index()
    return tmp
...
for scope in GNS3_scopes:
    scope.pcap_df()
    scope.adjust_time_without_scaling()
...
# set isp filter for access resolver
# Access_resolver.set_filter(isp_filter)
...
#             assert dst_arr[single_user].ndim == 2
            ...
            best_features = iterate_features(src_df, dst_df, n, features, tda_config,
                                            "./with-dot-change-unscaled/" + "chatlog_tda_match_dns_all_" + str(n) +
